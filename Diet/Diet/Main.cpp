...
int main(int, char**)
{
    ...
    GLFWwindow* window = glfwCreateWindow(880, 610, "Diet PLanner", NULL, NULL);
    if (window == NULL)
        return 1;
    ...
    int daily_goal = 0;
    int exit_val = 0;
    std::fstream MyFile("daily_goal.txt");
    if (MyFile)
    {
        if (MyFile.is_open()) {
            MyFile >> daily_goal;
    ...
    MyFile.close();
    std::ifstream read_cal("daily_calorie.txt");
    std::string myText = "";
    ...
    while (!glfwWindowShouldClose(window))
    {
...

        {
            ...
            ImVec2 desig1(328, 20);
            ImGui::SetCursorPos(desig1);
            ImGui::PushFont(title_font);
            ImGui::PushStyleColor(ImGuiCol_Text, IM_COL32(0, 255, 0, 255));
            ImGui::Text("|");
            ImGui::PopFont();
            ...
            ImVec2 desig2(328, 30);
            ImGui::SetCursorPos(desig2);
            ImGui::PushFont(title_font);
            ImGui::PushStyleColor(ImGuiCol_Text, IM_COL32(0, 255, 0, 255));
            ImGui::Text("|");
            ImGui::PopFont();
            ...
            ImVec2 desig3(328, 40);
            ImGui::SetCursorPos(desig3);
            ImGui::PushFont(title_font);
            ImGui::PushStyleColor(ImGuiCol_Text, IM_COL32(0, 255, 0, 255));
            ImGui::Text("|");
            ImGui::PopFont();
            ...
            ImVec2 desig4(328, 52);
            ImGui::SetCursorPos(desig4);
            ImGui::PushFont(title_font);
            ImGui::PushStyleColor(ImGuiCol_Text, IM_COL32(0, 255, 0, 255));
            ImGui::Text("|");
            ImGui::PopFont();
        ...
        static const char* items[]{ "Apple","Banana","Bagel","Biscuit","Breadsticks","Bread","Beans","Carrot","Corn","Cereal","Cookie","Cornbread","Grapes","Orange","Peas","Potato","Pizza","Popcorn","Pear","Pineapple","Raisins","Raspberries","Rice","Strawberries","Sweet-Potato","Spaghetti","Waffles","Watermelon"};
        int cal_te = 0;
        ...
        ImGui::Checkbox("Delete Data", &show_another_window);

        ImVec2 button_sz4(92, 38);
        if (ImGui::Button("New user", button_sz4))
        {
            new_user();
            del();
            daily_goal = 0;
        ...
        ImVec2 button_sz3(92, 38);
        if (ImGui::Button("Exit", button_sz3)) 
        {
            break;
        }
        if (daily_goal == 0)
        {
            std::fstream MyFile2("daily_goal.txt");
            if (MyFile2.is_open()) {
                MyFile2 >> daily_goal;
            }
            MyFile2.close();
        ...
        {
            std::fstream check_data("daily_calorie.txt");
            if (check_data)
            {
                check_data.close();
                ImGui::Begin("Delete Data", &show_another_window); 
    ...
    std::ofstream cal("daily_calorie.txt", std::ios::out | std::ios::app);
    {
